...
describe('DataSheet component', () => {
  ...
  const wrapper = mount(<Spreadsheet rows={data} columns={columns} closeWarningStatus={closeWarningStatus} airportCodes={airportCodeList} />)
  // afterEach(() => {
  ...
  it('renders without crashing', () => {
    ...
    ReactDOM.render(<Spreadsheet rows={data} columns={columns} closeWarningStatus={closeWarningStatus} airportCodes={airportCodeList} />, div);
  ...
  describe('check Form Control ', () => {
    ...
    it('mount', () => {
      ...
      const wrapper = mount(<Spreadsheet rows={data} columns={columns} closeWarningStatus={closeWarningStatus} globalSearchLogic={globalSearchLogic} airportCodes={airportCodeList} />)
  ...
  describe('check Sorting Panel ', () => {
    ...
    it('mount', () => {
      const sortingParamsObjectList = [{ sortBy: "FlightNo", order: "Ascending", sortOn: "Value" }];
      ...
      act((sortingOrderList) => {
        ...
        const modal = wrapper.props().setTableAsPerSortingParams(sortingParamsObjectList);
        expect(setTableAsPerSortingParams).not.toBeNull();
        const modal1 = wrapper.props().handleTableSortSwap(sortingParamsObjectList);
  ...
  describe('check Datagrid ', () => {
    ...
    it('mount', () => {
      ...
      const wrapper = mount(<Spreadsheet rows={data}
        ...
        {...songLinkProps} />)
      const getValidFilterValues = wrapper.find('ReactDataGrid').props().getValidFilterValues(columns[0].key);
      ...
      const onGridSort = wrapper.find('ReactDataGrid').props().onGridSort('flightno', 'DESC');
      expect(onGridSort).not.toBeNull();
      const rowGetter = wrapper.find('ReactDataGrid').props().rowGetter(0);
      expect(rowGetter).not.toBeNull();
      const onColumnResize = wrapper.find('ReactDataGrid').props().onColumnResize(0, 100);
      expect(onColumnResize).not.toBeNull();
      const onAddFilter = wrapper.find('ReactDataGrid').props().onAddFilter(filtermock);
      expect(onAddFilter).not.toBeNull();
      const onClearFilters = wrapper.find('ReactDataGrid').props().onClearFilters();
  ...
  describe('On Grid Row updated ', () => {
    ...
    it('mount', () => {
      ...
      const onGridRowsUpdated = wrapper.find('ReactDataGrid').props().onGridRowsUpdated({ "fromRow": 0, 'toRow': 0, 'updated': { flightno: "dss55555" }, "action": "CELL_UPDATE" });
  ...
  describe('On Row selected ', () => {
    ...
    it('mount', () => {
      ...
      const wrapper = mount(<Spreadsheet rows={data}
        ...
        selectBulkData={selectBulkData}
        maxLeftPinnedColumn={maxLeftPinnedColumn} />);
      const onRowsSelected = wrapper.find('ReactDataGrid').props().rowSelection.onRowsSelected(param);
      expect(onRowsSelectedfn).not.toBeNull();
      const onRowsDeselected = wrapper.find('ReactDataGrid').props().rowSelection.onRowsDeselected(param);
