...
const GridComponent = () => {
    ...
    // Options that has to be displayed in the kebab menu
    const rowActions = [
...
```jsx
// Passed rowActions will be displayed in the kebab menu against each row.
// Passed rowActionCallback will be called back on click of each row action, with rowData and row action value as parameters.
// Edit row action functionality will be handelled inside the Grid. getRowEditOverlay function will be called back where user has to provide a React component with editable fields of the row.
// Delete row action will be handelled inside the Grid. No call backs will be made in this case.
